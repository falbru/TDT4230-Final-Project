...
Mesh cube(glm::vec3 scale, glm::vec2 textureScale, bool tilingTextures,
          bool inverted, glm::vec3 textureScale3d) {
  glm::vec3 points[8];
  int indices[36];
  ...
  for (int y = 0; y <= 1; y++)
    for (int z = 0; z <= 1; z++)
      for (int x = 0; x <= 1; x++) {
        points[x + y * 4 + z * 2] =
            glm::vec3(x * 2 - 1, y * 2 - 1, z * 2 - 1) * 0.5f * scale;
      }
  ...
  for (int face = 0; face < 6; face++) {
    ...
    glm::vec2 textureScaleFactor =
        tilingTextures ? (faceScale[face] / textureScale) : glm::vec2(1);
...
Mesh generateSphere(float sphereRadius, int slices, int layers) {
  ...
  // Layers only requires angle varying between the bottom and the top (a layer
  // only covers half a circle worth of angles)
  ...
  for (int layer = 0; layer < layers; layer++) {
    ...
    // Angles between the vector pointing to any point on a particular layer and
    // the negative z-axis
    ...
    // The row of vertices forms a circle around the vertical diagonal (z-axis)
    // of the sphere. These radii are also constant for an entire layer, so we
    // can precalculate them.
    ...
    for (int slice = 0; slice < slices; slice++) {
      ...
      // Determining the direction vector for both the start and end of the
      // slice
      ...
      normals.emplace_back(radius * currentDirectionX,
                           radius * currentDirectionY, currentZ);
      normals.emplace_back(radius * nextDirectionX, radius * nextDirectionY,
                           currentZ);
      normals.emplace_back(nextRadius * nextDirectionX,
                           nextRadius * nextDirectionY, nextZ);
      normals.emplace_back(radius * currentDirectionX,
                           radius * currentDirectionY, currentZ);
      normals.emplace_back(nextRadius * nextDirectionX,
                           nextRadius * nextDirectionY, nextZ);
      normals.emplace_back(nextRadius * currentDirectionX,
                           nextRadius * currentDirectionY, nextZ);
      ...
      for (int j = 0; j < 6; j++) {
        ...
        uvs.emplace_back(0.5 + (glm::atan(vertex.z, -vertex.x) / (2.0 * M_PI)),
